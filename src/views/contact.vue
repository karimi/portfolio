<template>
    <div class="max-h-screen overflow-y-auto flex flex-col md:flex-row gap-12 mx-auto my-8 p-6 rounded-lg shadow-none">
        <div class="flex-1 flex flex-col">
            <h1 class="text-2xl font-bold mb-4">Contact</h1>
            <p class="mb-4">
                I'd love to hear from you whether it's about my work at McNeel, paintings, or just to say hi. I'm not on any social media, so the best way to reach me is via email.
                <br />
                <a href="mailto:email@mortezakarimi.com" class="text-blue-600 underline">email@mortezakarimi.com</a>
                <br />
                <p>LinkedIn: <a href="https://www.linkedin.com/in/mortza" class="text-blue-600 underline">linkedin.com/in/mortza</a></p>
                <p>GitHub: <a href="https://github.com/karimi" class="text-blue-600 underline">github.com/karimi</a></p>
            </p>
        </div>
    </div>
</template>
